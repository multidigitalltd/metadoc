...
### תמונות ניתנות להחלפה
- מקור אמת יחיד: `Metadoc_RealEstate::image_slots()` (חריץ = label/desc/file)
  ו-`image_groups()` (קיבוץ לתצוגה). מסך הניהול, ההתאמה האישית והנפילה
  לברירת מחדל נגזרים כולם מהם — אין רשימה כפולה.
- הוספת חריץ לתמונה מצורפת לתבנית: מוסיפים ערך עם `file` והתבנית מכבדת
  אותו אוטומטית דרך `metadoc_image()`, בלי לגעת בקבצי ה-template-parts.
- תמונה שהוחלפה מוגשת ב-`wp_get_attachment_image()` (srcset + מידות אמיתיות);
  מזהה שאינו תמונה נופל בחזרה לקובץ המצורף ולא שובר את העמוד.
