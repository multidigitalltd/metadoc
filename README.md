...
### תמונות
המסך הנוח להחלפה: **מחלקת נדל"ן → תמונות** בלוח הבקרה. כל החריצים מוצגים
שם כרשת כרטיסים עם תצוגה מקדימה, כפתור "בחירת תמונה" (ספריית המדיה),
"הסרה" ושמירה אחת לכל המסך. אותם חריצים זמינים גם ב**התאמה אישית →
מחלקת נדל"ן — תמונות** (עם תצוגה חיה) — שני המסכים כותבים לאותו מקום.

החריצים מחולקים לשתי קבוצות: תמונות עמוד המחלקה (Hero, אודות, התהליך,
רצועת CTA ושלושה כרטיסי פרויקט) ותמונות ברירת המחדל של עמוד הפרויקט
(Hero, הדמיית בינוי, מפת גבהים) — שכבר מצורפות בתבנית. לתמונה ייעודית
לפרויקט מסוים עורכים את הפרויקט עצמו. חריץ ריק מציג מסגרת מציין-מקום
ואינו שובר את הפריסה.
