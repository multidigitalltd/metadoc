<?php
/**
 * כרטיס פוסט (תוכן פנימי) לרשת הבלוג/ארכיון. נקרא בתוך הלולאה.
 *
 * @package Metadoc
 */
...
<article <?php post_class( 'group bg-white border border-neutral-200 rounded-2xl overflow-hidden flex flex-col hover:shadow-[0_24px_60px_-30px_rgba(0,0,0,0.25)] hover:border-neutral-300 transition-all' ); ?>>
	<a href="<?php the_permalink(); ?>" class="block overflow-hidden aspect-[16/10] bg-neutral-100" tabindex="-1" aria-hidden="true">
	...
	<div class="p-6 flex flex-col flex-1">
		...
		<h2 class="font-bold text-xl leading-tight tracking-tight text-neutral-900 font-display mb-2">
			<a href="<?php the_permalink(); ?>" class="hover:text-[#d96a10] transition-colors"><?php the_title(); ?></a>
		</h2>
		<p class="text-[14px] text-neutral-600 leading-relaxed flex-1"><?php echo esc_html( wp_trim_words( get_the_excerpt(), 24 ) ); ?></p>
		<a href="<?php the_permalink(); ?>" class="inline-flex items-center gap-1.5 mt-4 text-[14px] font-extrabold text-neutral-900 group-hover:text-[#ff7a00] transition-colors">
			<?php esc_html_e( 'קראו עוד', 'metadoc' ); ?>
